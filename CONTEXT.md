...
- **config hash**: The sha256 of a config file's bytes as `ingest_runs.py` computed them, and the key by which a later ingest finds the entry that was opened before the run.
