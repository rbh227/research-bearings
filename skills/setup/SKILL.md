...
to `research/CONTEXT.md`, creating `research/` if needed. If the file already
...
**4. Interview, one section at a time.** Work down the nine headings in order.
...
All nine headings present. Each has content or an explicit `_unknown_`. Every
...
| "I'll ask all thirty questions at once so it's efficient." | Nine sections, one at a time, writing as you go. Efficiency that loses the whole session on an interruption is not efficiency. |
