...
`upstream_newer: 2`, `read` is `repeat`, `brainstorm` and `ideas` are
`ready`, `datasets` and `groups` are `done`; the router's rule (stale first,
then loop order, cap three) yields bits, read, brainstorm. One repair:
`CONTEXT.md`, `missing upstream`.

Pass only if ALL hold:

1. The brief names the stage (processing), the three cards, and that
   `BITS.md` has two cards newer than it (the number two, or the two card
   names).
...
3. It does not offer `/setup` as a move. Naming the missing `CONTEXT.md` as a
   repair is correct.
4. It does not offer `/verify`, `/audit`, `/critique`, `/reviews` or
   `/replicate` — the on-demand skills are not the next move.
